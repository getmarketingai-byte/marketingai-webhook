 ...
 * Lazy-loads nodemailer to avoid startup failures.
 */
...
function verifyStripeWebhook(rawBody, sig, secret) {
  ...
  if (expected.length !== parts.v1.length || !crypto.timingSafeEqual(Buffer.from(expected), Buffer.from(parts.v1)))
...
function getTransport() {
  // Lazy load nodemailer to avoid startup failure
...
async function sendWelcomeEmail(toEmail, toName) {
  ...
  const lines = [
    ...
    '',
    '1. AI Content Engine',
    '   30-day LinkedIn content calendar. 10 posts drafted, 4 themes, copy-paste ready.',
    '',
    '2. Outbound Lead Sequence',
    '   Personalised LinkedIn connection request + 2 DM follow-up templates.',
    '',
    '3. Email Nurture Sequence',
    '   3 emails: intro, value, and a low-pressure CTA -- formatted and ready to load.',
    '',
...
async function sendAdminNotification(clientEmail, clientName, amountCents, paymentId) {
  ...
  const lines = [
    ...
    '',
    'Welcome email sent. Client will book via Calendly.',
    '',
    '-- MarketingAI Webhook',
  ];
  await transport.sendMail({
...
module.exports = async (req, res) => {
  if (req.method === 'GET') {
    // Health check -- verify nodemailer loads
    let nodemailerOk = false;
    try { require('nodemailer'); nodemailerOk = true; } catch (e) { /* ignore */ }
    return res.status(200).json({ status: 'ok', service: 'marketingai-webhook', nodemailer: nodemailerOk });
  }
  ...
  let event;
  try {
    event = verifyStripeWebhook(rawBody, req.headers['stripe-signature'], process.env.STRIPE_WEBHOOK_SECRET);
  } catch (e) {
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      ...
      if (s.payment_status !== 'paid') return res.status(200).json({ received: true, skipped: 'not paid' });
      const email = (s.customer_details && s.customer_details.email) || s.customer_email;
      const name = (s.customer_details && s.customer_details.name) || '';
      if (!email) return res.status(200).json({ received: true, warning: 'no email' });
      ...
      const name = (intent.shipping && intent.shipping.name) || (intent.metadata && intent.metadata.name) || '';
      if (email) {
        await sendWelcomeEmail(email, name);
